...
class Evaluator:
    ...
    def generate_odoroki(self, row: pd.Series) -> str:
        prompt_body = pog.generate(
            news=row.news_description, character=row.odoroki_persona
        )
...
def main(num_samples: int) -> None:
    ...
    if len(list(DATA_DIR.glob("interim/**/*.csv"))) == num_samples:
        logger.info(
            "Already odoroki exists. Please delete files if regeneration required."
        )
    else:
    ...
    for i, file in enumerate(DATA_DIR.glob("interim/**/*.csv")):
        ...
        if i == 0:
            if news_path.exists():
                logger.info(
                    "Already odoroki embedding exists. "
                    "Please delete files if regeneration required."
                )
            else:
        ...
        if DATA_DIR.joinpath(f"processed/generated_embedding_{index}.npy").exists():
            logger.info(
                f"Already emgedding for {index} exists. "
                "Please delete files if regeneration required."
            )
        else:
            ...
            np.save(
                DATA_DIR.joinpath(f"processed/generated_embedding_{index}"),
                generated_embedding,
            )
    ...
    for file in DATA_DIR.glob("processed/generated_embedding_*.npy"):
        ...
        _news_vs_generated_all.append(
            evaluator.calculate_distance(news_embeddings, generated_embeddings)
        )
        _odoroki_vs_generated_all.append(
            evaluator.calculate_distance(odoroki_embeddings, generated_embeddings)
        )
    ...
    for i in range(odoroki_vs_generated_all.shape[1]):
        ...
        logger.info(
            f"\tMax similarity for data {i + 1} is {max_similarity:.2f} and file {max_index + 1}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_samples",
        help="number of samples to calculate vector",
        type=int,
        default=5,
    )
